...
def cleanup():
    ...
    if (cnt != 0):
        print wht("Removed ", 1)+err(str(cnt))+wht(" autogenerated lines from .bashrc")
...
def gen_launcher(bash_name, launcher_name, icon_name, command):
    ...
    if len(str(NUCOBOT_BASE)) != 0:
        format_str +='export NUCOBOT_BASE=' + str(NUCOBOT_BASE) + '\n'
    if len(str(NUCOBOT_3D_SENSOR)) != 0:
...
parser = argparse.ArgumentParser(description='Deploy script for NUCOBOT project.')
